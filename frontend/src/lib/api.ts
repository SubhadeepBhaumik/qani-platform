...
const API_BASE = 'http://139.180.181.11:5001/api/v1';
...
export const api = {
  ...
  applyForJob: async (jobId: string, candidateId: string): Promise<Application> => {
    return call<Application>('/applications', {
      method: 'POST',
      body: JSON.stringify({ jobId, candidateId, status: 'applied' }),
    });
  },
  ...
  startScreening: async (applicationId: string): Promise<ScreeningSession> => {
