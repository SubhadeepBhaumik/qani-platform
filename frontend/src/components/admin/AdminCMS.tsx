...
function useCMSStore() {
  ...
  const cmsRef = useRef<any>(DEFAULT_CMS);
  ...
  useEffect(() => {
    ...
    fetch('https://qani.io/api/v1/admin/cms', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(res => {
        if (res.success && res.data) {
          const merged = { ...DEFAULT_CMS, ...res.data };
          cmsRef.current = merged;
          setCms(merged);
  ...
  const save = useCallback(() => {
    const current = cmsRef.current;
    localStorage.setItem(CMS_KEY, JSON.stringify(current));
    const token = localStorage.getItem('qani_auth_token');
    fetch('https://qani.io/api/v1/admin/cms', {
      method: 'POST',
...
export function AdminCMS() {
  ...
  const setActivePageWithUrl = (page: CMSPage) => {
    setActivePage(page);
    ...
    <div className="flex" style={{ minHeight: '80vh' }}>
      <div className="w-52 bg-gray-900 border-r border-gray-800 flex flex-col shrink-0 rounded-l-xl">
        ...
        <nav className="flex-1 p-2 overflow-y-auto">
          {PAGES.map(page => (
            <button key={page.id} onClick={() => setActivePageWithUrl(page.id)}
